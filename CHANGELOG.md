...
- **修订状态中文化**：`revisions.md` 的范围状态与运行状态改为“中文（ENUM）”双语展示，保留精确枚举便于与 MCP、SQLite 和故障诊断对照。
- **完整枚举说明**：修订历史新增范围状态与运行状态对照表，分别列出 7 个合法状态及其中文名称和语义，补齐 `ABANDONED`（已放弃）翻译，并明确范围冻结与 Graph Run 生命周期是两套独立状态。
- **兼容性与验证**：仅改变人类 Markdown 投影，不改变 MCP 工具、schema v3、SQLite 或运行协议，无数据迁移；全量 Python 456 项完成（455 通过、1 项按环境跳过），全树编译、四个 Skill、Codex/Claude/ZCode Plugin、release candidate、镜像与差异校验通过。
...
- **短租约与实时心跳面板**：Loop 有效租约缩短为 5 分钟；receiver 每 60 秒发送心跳，进入剩余 2 分钟的续租窗口后延长租约。派遣后立即在 Agent 主会话输出进度面板，后续心跳更新实时面板但不重写 Markdown 投影，投影只在关键进度和终态节点更新。
- **调度边界收敛**：删除基于 Agent 风险分类的保障档推荐工具；`STANDARD` 成为默认档位，`LIGHT` 只接受用户明确选择。
- **删除额度状态机**：移除 provider 额度阈值、宿主共享断路器、定时恢复、相关 SQLite 字段/表与 frontier action；`pause_loop` 只保留显式暂停，`resume_loop` 负责显式恢复。
- **删除模型与推理遥测**：assignment、claim、Graph 状态、进度投影和 outcome schema 不再接收或展示模型、推理档位与 Worker 遥测。固定并发槽位、短租约、心跳和资源锁保持不变。
- **协议影响**：MCP 工具联集由 33 减为 32，planning Profile 由 16 减为 15；新建 schema v3 状态不再包含额度持久化结构。
- **升级说明**：既有 schema v3 数据库中的旧额度列或表可以原样保留，当前运行时不再读取或写入；宿主升级 Plugin 后应新建会话以刷新 32 个工具的 Profile。
...
- **README 回归项目摘要**：首页从 176 行压缩为 74 行，删除历史演进残留和大段状态、恢复、Git、调度与验收协议，只保留项目定位、核心能力、4 个 Skill / 3 个 MCP Profile、最短使用方式、安全边界和深入文档入口。
- **Marketplace 安装边界**：README 不再复制包含内部仓库地址的安装命令，安装、升级与回滚统一路由到团队运维文档，避免把内部 Marketplace 配置混入公开项目摘要。
- **兼容性**：Controller、33 个 MCP 工具、四个 Skill、三个 MCP Profile、schema v3、宿主行为和 `.layered-delivery/` 数据 namespace 均无变化，已有 Graph 无需迁移。
- **验证**：全量 Python 427 项完成（426 通过、1 项按环境跳过），并完成 `compileall`、四个 Skill、Claude Plugin、release candidate 与差异校验。
