...
需求分析可以从任意一个项目开始，但开发授权不能只包含当前目录。只要冻结任务需要修改多个目录、仓库或微服务，宿主就选择一个协调工作区保存 `.ai-dev-loop/<task-id>/`，并在当前轮次生成：
