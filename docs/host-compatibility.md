...
当前矩阵只验证同宿主原生派遣：Claude 终端的全部 claim 必须是 `claude-code`，Codex 终端的全部 claim 必须是 `codex`。即使 PATH 发现另一 CLI，也不得加入 `HOST_NATIVE` inventory。可信多 Adapter 桥接尚未实现，因此 0.33.2 不建立跨 Agent 冒烟任务；未来实现后应作为独立矩阵维度增加，不能复用当前结果宣称支持。
...
上述版本是 0.33.2 候选基线，不是永久兼容承诺。宿主升级后若 Hook 事件字段、Plugin manifest 或 MCP 工具命名发生变化，应先在自托管 Runner 重跑真实宿主冒烟，再更新本矩阵。
