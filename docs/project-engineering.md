...
`scheduler.db` 是需求与调度状态的机器权威；每个需求只使用稳定的 `.layered-delivery/<delivery-id>/`。自动与手动开发都生成根 `overview.md` 以及 Delivery 的 overview、baseline、progress、acceptance、revisions 和同结构 work-items，手动路径另含 `handoff-<fingerprint>.md`。手动包先以 `HANDOFF_READY` 登记到 SQLite；交接阶段不创建 `runs`、事件链或 workspace 绑定，`requirementSnapshotStatus=FROZEN` 只表示内容由双 fingerprint 锁定。接收 CLI 在实际工作区调用 `start_manual_handoff` 后，Controller 采用同一 Revision、绑定 workspace 并创建 `execution_mode=manual` 的 run；TASK_LOOP 只能 `dispatch_mode=MANUAL`，所有 TASK/GROUP/Delivery Review 只能 `AUTO`，最终仍需用户确认。此后 progress/acceptance 只由事件投影刷新，不能人工维护。稳定 `delivery.id` 下可以有多个不可变 Revision；`revisions.md` 默认展示当前 Revision 并串联旧 run 的 `SUPERSEDED` 审计状态。新用户需求不能因为当前路径恢复了旧 Delivery 就隐式进入其 Revision；`prepare_delivery_revision` 必须提交 `USER_EXPLICIT_SAME_DELIVERY`，或在已有 `REPLAN_REQUIRED` 事件时提交 `ACTIVE_LOOP_REPLAN`。候选只写 `delivery_revisions`，不会移动 `hierarchies` 当前指针，旧 run 直到候选 freeze 时才被原子取代。同一 `workspaceKey` 已有未结束 run 时，第二个 Delivery 在 `prepare` 写入前即被拒绝并返回 `CREATE_INDEPENDENT_WORKTREE_TASK` 与宿主路由 `worktreeSetup`，避免留下无法冻结或迁移的 PREPARED 状态。Claude Code 裸 CLI 可在当前 primary checkout 以 `EXCLUSIVE_PRIMARY_CHECKOUT` 绑定一个未结束 Delivery；有效 feature 分支直接就绪，主线或 detached 状态取得 Git 授权后在当前 checkout 建立 Delivery feature 分支并由同一会话续接。Codex primary 的 `worktreeSetup.hostDispatch` 提供立即启动策略、宿主操作、确定性 idempotency key、基线和完整续接参数，并创建 `environment=worktree` 项目任务；并行或占用的 Claude Delivery 也使用新的 linked worktree 会话。Controller 不执行 Git 写操作。`projectScopes` 允许一个需求覆盖多个本地仓库，冻结时要求精确项目 ID 授权；所有可写 Git 项目使用同名 feature 分支，但分别绑定自己的主线与 `baseCommit`。TASK Agent 不创建内部分支；Git stage/commit/push 仍需各自授权。`work-items/<root-id>/children/...` 镜像逻辑父子关系，但不表达文件授权。

Git 宿主支持两种策略：Claude Code 裸 CLI 的单 Delivery primary 路径使用 `EXCLUSIVE_PRIMARY_CHECKOUT`；Codex 与并行/占用的 Claude Delivery 使用 `HOST_NATIVE_LINKED_WORKTREE`。Controller 不执行 Git 写操作，而由 `workspace_status.worktreeProvenance` 记录实际宿主、策略、拓扑、`selectionSource`、`baseRef`、`baseCommit`、`baseHeadCommit` 与 `integrationTarget`。只有未被其他 worktree/Delivery 使用、基线有效的 feature 分支才可 adoption；已有业务 diff 必须由用户确认全部属于本 Delivery，再以精确 `confirmed_dirty_state_fingerprint` 重查，`.layered-delivery/**` 控制面产物不计入 dirty 状态。Claude primary 的主线或 detached HEAD 返回当前 checkout 的建分支动作；Codex primary 返回可由宿主消费的 `hostDispatch`。
...
- 规划与交接：`workspace_status`、`hierarchy_contract`、`preview_hierarchy`、`select_execution_mode`、`resume_execution_mode`、`create_manual_handoff`、`start_manual_handoff`、`prepare_hierarchy`、`freeze_hierarchy`。`workspace_status(base_ref=...)` 可承接宿主明确选择的基线；未指定时按有效 `origin/HEAD`、本地 `main`、本地 `master` 降级发现，并在 `worktreeProvenance` 中返回来源与精确提交。Claude primary 或 linked worktree 的主线/detached 状态返回 `CREATE_DELIVERY_FEATURE_BRANCH`，建好本地 feature 分支并重查后才提供 `suggestedGitBinding`。preview 先登记 `CHOICE_READY` 并生成数据库、总览、基线和全部关联投影，再返回 schema v2 `executionChoice`：Codex 映射 `request_user_input`，Claude 映射 `AskUserQuestion`，当前上下文可调用时必须使用原生选择器；只有不可调用时才允许逐字 Markdown 降级，Agent 不得改写或要求用户输入选项文字。`select_execution_mode` 先记录 Controller 返回的自动/手动选项；AUTOMATIC 在 Claude primary 上由同一会话创建 feature 分支并调用 `resume_execution_mode`，Codex primary 则通过 `hostDispatch` 创建 worktree 项目任务后续接同一次确认。手动开发在本需求的 `<delivery-id>` 目录生成完整冻结内容包，并必须用 `start_manual_handoff` 把该快照接入完整 Graph。
...
`controller.py` 是唯一共享应用入口；`mcp_tools.py` 把 29 个模型可调用工具映射到 Controller。receiver 凭证签发和硬额度熔断是模型外宿主回调，不进入 MCP 工具目录；工具数量由契约测试与真实目录同步校验。`mcp_adapter.py` 负责协议、精确启动 Adapter 身份与宿主策略。Controller 不扫描 PATH、不执行本机 CLI 版本探针，也不提供 Agent/模型发现或中央设置工具。
