...
- 同一个实际物理 workspace 可以绑定多个 Delivery，状态始终以显式 `rootId` 路由；执行策略只有 `CURRENT_WORKSPACE_SERIAL`。已有调度 owner 时，后启动的 AUTOMATIC 或 MANUAL Delivery 都持久记录为 `QUEUED` 并投影队列状态。`PAUSED`、Run 终态或 `RECORD_USER_CONFIRMATION` 只表示进入 release eligibility，不等于物理 turn 已释放；所有 READ_WRITE scope 必须在各自冻结的独立分支上形成 turn start 之后的可验证业务 commit，并保持 working tree/index clean、HEAD 与 binding 匹配且 receiver/reservation 安全收束。Controller 原子复核全部 scope 并持久化 `WORKSPACE_TURN_RELEASED` 后，宿主才能切分支和续调队首；标准顺序固定为 `quiesce → eligible state → commit/clean on frozen branch → protocol recheck/persist release → branch switch/next run`，不得创建新 worktree 绕过。待用户确认只释放物理 turn，不标记 `COMPLETED`；暂停后恢复必须重新排队，轮到并记录新的 clean turn start 后才可重新领取。最终确认可在 release 已持久化后从新分支按旧 `rootId` 补录。若用户在验收边界提出修改，下一 Revision 重新排队并在轮到后捕获新的 clean turn start。
