...
- 入口与外层 receiver 派遣：`route_entry_intent` 先排除被否定的生命周期动作，再把唯一肯定入口文本与持久化状态合成为确定性路由；多个肯定动作失败关闭为 `AMBIGUOUS`。`CANCELLED + OPEN + WORKSPACE_TURN_RELEASED` 的继续/修订只产生 `NEXT_REVISION_REQUIRED` planning 路由，恢复旧 Run 仍失败关闭。可选 decision-only Supervisor 默认关闭且无工具权限。`plan_dispatch_batch` 按当前宿主 Adapter、版本化 Agent Profile Catalog、profile 并发槽位和 frontier 直接预留 receiver，不接收模型或推理档位字段。
