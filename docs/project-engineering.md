...
- 规划与交接：`workspace_status`、`hierarchy_contract`、`preview_hierarchy`、`select_execution_mode`、`resume_execution_mode`、`create_manual_handoff`、`start_manual_handoff`、`prepare_hierarchy`、`freeze_hierarchy`。`workspace_status(base_ref=...)` 可承接宿主明确选择的基线；未指定时按有效 `origin/HEAD`、本地 `main`、本地 `master` 降级发现，并在 `worktreeProvenance` 中返回来源与精确提交。detached linked worktree 返回 `CREATE_DELIVERY_FEATURE_BRANCH`，建好本地 feature 分支并重查后才提供 `suggestedGitBinding`。preview 先登记 `CHOICE_READY` 并生成数据库、总览、基线和全部关联投影，再返回 schema v2 `executionChoice`：Codex 映射 `request_user_input`，Claude 映射 `AskUserQuestion`，当前上下文可调用时必须使用原生选择器；只有不可调用时才允许逐字 Markdown 降级，Agent 不得改写或要求用户输入选项文字。`select_execution_mode` 先记录 Controller 返回的自动/手动选项；AUTOMATIC 在 primary checkout 返回 `hostDispatch`，宿主自动创建 worktree 任务，随后由 `resume_execution_mode` 在实际 linked worktree 续接同一次确认。手动开发在本需求的 `<delivery-id>` 目录生成完整冻结内容包，并必须用 `start_manual_handoff` 把该快照接入完整 Graph。
