...
- 前序 Delivery 到达 `PAUSED`、Run 终态或 `RECORD_USER_CONFIRMATION` 只形成 release eligibility；被取消 receiver 的租约必须结束且没有 reservation，所有 READ_WRITE scope 都要在各自冻结独立分支形成相对 turn-start 的非空业务 commit，并保持历史未改写、工作树/index clean 与 binding 匹配。`CANCELLED` 且全部 scope 零业务变化、clean、HEAD 等于 turn-start 时可用零变化证据释放。Controller 持久化 `WORKSPACE_TURN_RELEASED` 后才释放物理 turn；待用户确认只释放 checkout，不标记完成。PAUSED turn 未释放时可由新 receiver 原地恢复，已释放时才重新排队并重获 turn。错分支、dirty、HEAD 或 scope 漂移全部失败关闭。
- `CANCELLED` Run 永不由旧 receiver 或新 receiver 恢复。若 Delivery 仍 `OPEN` 且 workspace release 已持久化，明确继续/修订通过 `route_entry_intent → prepare_delivery_revision → freeze/create_manual_handoff` 创建下一不可变 Revision；未释放时先完成 release，不能用业务修改、空提交、伪造 handoff 或直接改 SQLite 解锁。
