...
Claude 命令从当前 0.40.2 源码发布包的 `--plugin-dir` 加载 Plugin；Codex 命令要求候选 Plugin 已从 Marketplace 安装；ZCode 由已安装 Plugin 的真实会话加载。发布前可用 LIGHT 验证 `recommend_assurance_profile → select_execution_mode → 当前 checkout 分支准备 → resume_execution_mode → plan_dispatch_batch → 独立 TASK child → dispatch_loop(AUTO) → result`；短任务允许没有显式 heartbeat/progress。再用 STANDARD 覆盖首次 heartbeat、进度和独立 Review。Claude 由主会话在 `CURRENT_WORKSPACE_SERIAL` 边界内协调当前分支和独立 receiver Agent，不启动专用后台 coordinator。任何输出中的 `claimedAgents` 都只能包含命令指定的当前宿主。
