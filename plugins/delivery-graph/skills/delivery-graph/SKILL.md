...
- 一个实际 workspace/worktree 可以绑定多个未结束 Delivery，控制状态始终以 `rootId` 隔离，但工作区策略只有 `CURRENT_WORKSPACE_SERIAL`：每个 Delivery 使用独立分支，同一物理 checkout 一次只运行一个 Delivery。已有调度 owner 时，后启动或后发现的自动 Delivery 标记为 `QUEUED`，保留无需再次确认的自动 continuation；前一个进入 Run 终态，或已到 `RECORD_USER_CONFIRMATION`，并形成可验证业务 commit、working tree/index clean、HEAD 未漂移且 receiver/reservation 全部释放后，即可释放物理 workspace turn，未验收 Delivery 仍保持待用户确认。随后自动准备和续调队首。owner dirty、资源冲突、未合并状态或 HEAD 漂移时停止切换。禁止跨 Delivery 共享 checkout/branch 并行执行。新业务目标默认创建新 Delivery；只有同一需求延续或 `REPLAN_REQUIRED` 才创建同一 Delivery 的 Revision。
...
- 选择 `AUTOMATIC` 时只使用 `CURRENT_WORKSPACE_SERIAL`，不创建或预留新 worktree。已有 owner 时，响应与投影把后启动 Delivery 标记为 `QUEUED`；保存其 `deliveryQueue.continuation`。前一个 Delivery 进入 Run 终态，或到达最终用户确认边界，并产生可验证业务 commit、working tree/index clean、HEAD 与冻结 binding 一致且 receiver/reservation 全部安全释放后自动续接，不重选模式。人工 Graph 达到相同边界时也允许宿主在已获 Git 授权后 commit 并切换新 Delivery 分支。
- 同一 Delivery 冻结任意后续 Revision（`N → N+1`）时，若原物理 turn 尚未释放，项目集合、checkout、分支与冻结基线未变，Controller 复用最初的 clean `workspaceTurnStart`，允许前序 Revision 已产生的 tracked、staged 或 untracked 业务改动原地进入下一 Revision；不得要求用户删除生成物、stash 或创建检查点提交，且 Revision 确认仍不授权 commit。若此前已在最终用户确认边界安全释放 turn，用户提出修改时同一 Delivery 的下一 Revision 必须重新进入队列，轮到后切回冻结分支并捕获新的 clean turn start；不得抢占当前 Delivery。未解决冲突、原始 turn 历史被改写，或项目/绑定变化时继续 fail closed，并按返回的清洁边界处理。
...
- `REFREEZE_TASK_REQUIREMENT`：停止派遣该 TASK，先对受影响 TASK 重新执行切分完整性预检，再只提交用户授权的完整替代 requirement。`refreeze_task_requirement` 以 SQLite 中当前不可变 hierarchy 为权威，把替代内容重锚为同一 Delivery 的下一 Revision，保留旧 Revision 双指纹并沿用现有执行模式；不得原地更新旧 Revision、从 Markdown 反推 Graph，或复用旧 assignment。任一当前 Run 的未领取 dispatch reservation 都绑定旧 Graph 指纹；两个需求修订工具返回 `SCHEDULER_TASK_REQUIREMENT_RESERVATION_ACTIVE` 时，等到其 `retryAfter` 后再续接。
...
- 跨 Delivery 出现相同 `resourceClaims` 或物理工作区冲突时，只把已选择 `AUTOMATIC` 的后启动或后发现 Delivery 标记为 `QUEUED`；手动冻结保持 `HANDOFF_READY`。前一个 Delivery 进入 Run 终态，或已到 `RECORD_USER_CONFIRMATION`，并形成可验证业务 commit、工作树 clean、HEAD 未漂移且 receiver/reservation 安全释放后即可自动续调队首；最终用户确认仍可稍后按原 `rootId` 补录，不要求切回旧分支。任一条件不满足就保持排队。`CANCELLED` 的安全释放不依赖归档，终态查询不得用过期 `workspaceRebase` 阻塞新 Delivery；归档仍只隐藏已完成历史。资源无交集也不能绕过 `CURRENT_WORKSPACE_SERIAL` 的单 checkout 串行边界，且绝不能 stash 正在运行 owner 的未完成改动。
...
- frontier 返回 `RECORD_USER_CONFIRMATION` 时，展示分层验收结果并等待真实用户确认。此时若业务变更已 commit、工作区干净且没有 receiver/reservation，允许先释放物理 workspace turn 并切换其他 Delivery；`record_user_confirmation` 是控制面确认，可在新分支上按旧 `rootId` 补录，不能再次要求旧分支 checkout。
