...
用于自动 Graph，以及手动 handoff 接收后启动的同一冻结 Graph 的运行、恢复与阻断处理。两种模式共享 Hook 认证、claim、项目 scope、operation、heartbeat、资源锁、进度、Review、验收和恢复契约；差异只在 TASK 实现的授权来源。Codex AUTOMATIC TASK 由 `SessionStart` 认证的当前 Delivery 会话直接领取，AUTO Review 仍绑定非空 reservation 和独立 receiver；MANUAL attestation 的 reservation 固定为 `NULL`。
...
2. Codex READY TASK 以当前 Graph fingerprint 调用 `claim_current_task`；Controller 验证 session capability、Delivery worktree、TASK kind、READY attempt 和项目 scope 后，以 `INLINE_AUTO/HOST_SESSION` claim，完全不创建 reservation。任何 Review 都拒绝该工具，改为调用一次 `plan_dispatch_batch` 并按 `concurrentDispatchGroups` 创建独立 receiver。code mode 内嵌 MCP 即使当前宿主未触发嵌套 PreToolUse，也必须携带 `SessionStart` 能力才能 planning、claim 或 mutation；没有 Hook 时返回 `SCHEDULER_HOST_HOOK_NOT_READY`。能力不得复制给 Worker。
3. `claim_current_task` 成功后当前会话立即读取 `loop_context` 并 heartbeat，不再创建 child。对 Review assignment 仍先预留、再立即创建 receiver；Codex 由 `SubagentStart` Hook 在 child 可见前完成 host-side claim，并向该 child发放自己的 session capability。Hook 启动证明失败时沿用 reservation 立即释放和 `DELIVERY_GRAPH_STARTUP_ERROR` 规则。若当前 TASK session capability 缺失，不得退回无认证直领；先信任 Hook 并新建/恢复 Delivery 任务。
