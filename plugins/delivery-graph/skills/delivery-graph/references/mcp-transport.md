...
Codex Plugin manifest 必须显式声明 `./hooks/hooks.json`。安装或升级后在 `/hooks` 中审查并选择始终信任当前精确 Hook 定义；信任按内容哈希持久保存，未变更时后续任务不再弹，升级改变 Hook 后重新审查。`SessionStart` 在顶层 Delivery task 启动、恢复或 compact 时为精确 session/worktree 签发可轮换、有时效、数据库只存哈希的 coordinator capability；启动发生在自动选择冻结前时，Hook 可依据已记录的 AUTOMATIC selection 预签发，后续 claim 仍必须通过最终 workspace binding。subagent 的普通 SessionStart 不签发该角色。当前 receiver 在 code-mode MCP 调用中携带 Hook 私有字段；Adapter 同时核对 capability、session role、可信请求 workspace 和 Delivery binding。缺少、伪造、跨 role/session/workspace 或过期时 fail closed。该路径不要求修改 `model_catalog_json`、模型 tool mode 或用户级编排配置。

Codex AUTOMATIC TASK 由当前 SessionStart capability 调用 `claim_current_task`，以 `INLINE_AUTO/HOST_SESSION` claim 且没有 reservation；该工具只接受 READY `TASK_LOOP`。AUTO Review 由 reservation 与 `SubagentStart` 授权，reservation 非空；MANUAL TASK 仍由 reservation 为 `NULL` 的 receiver attestation 授权。三条路径共享 operation、lease 和 mutation 校验。Codex transcript 必须位于宿主可信 `.codex/sessions` 根；session capability 不得复制给 root/helper、内部 Worker、自定义 `CODEX_HOME` 或其他任务。
...
已激活 AUTOMATIC Graph 的 Codex TASK 默认由当前 Hook-attested Delivery 会话直接 claim，不再因 `SubagentStart` 无法创建实现 receiver。若 `SessionStart` capability 缺失，先信任当前 Hook 并新建/恢复 Delivery 任务；禁止无认证直领。`handoff_ready_automatic_task` 只保留为其他 Adapter 或显式 MANUAL 接管的兼容恢复，Review 继续自动独立派遣。
