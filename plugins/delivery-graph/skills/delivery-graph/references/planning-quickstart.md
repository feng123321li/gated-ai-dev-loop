...
- 当前 Run 已 `CANCELLED` 时绝不恢复该 Run。只有 Delivery 仍为 `OPEN/未上线`、Controller 已持久化 `workspaceRelease=RELEASED`，且用户明确继续或修订同一 Delivery，`route_entry_intent` 才以 `NEXT_REVISION_REQUIRED` 进入本 Skill；随后保持原 `delivery.id` 调用 `prepare_delivery_revision` 创建下一不可变 Revision，再按该 Revision 的确认模式冻结。release 尚未完成时只执行释放动作，不得准备 Revision、切分支或修改业务代码。
...
8. 用户点选执行方式后，把选项 ID、双 fingerprint、`requiredProjectAuthorizations` 的精确项目 ID 和真实确认人一次性传给 `select_execution_mode`：
   ...
   - `MANUAL`：生成同结构开发包和自包含 handoff，登记 `HANDOFF_READY`；执行模式选择时已记录当前 workspace 队列绑定，但交接阶段不创建 Graph Run、receiver、任务或 worktree。宿主展示 `manualHandoff.receiverPrompt`，不得改写；同一提示词已经嵌入 `manualHandoff.path` 指向的文件。接收宿主必须先调用 `start_manual_handoff`，再创建独立原生 child；child 以显式 receiving context 和新 `operation_id` 按 frontier MANUAL claim TASK，不携带 AUTO reservation、decision fingerprint、Profile 或 Team 决策字段。Review 不允许 MANUAL，必须走统一 AUTO 预留与派遣。
