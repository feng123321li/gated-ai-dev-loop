...
def manual_receiver_prompt(
    ...
    base_prompt = (
        ...
        "operation_id；每个 child claim 后立即首次 heartbeat，即使返回 "
        "NOT_REQUIRED 也保留原 expiresAt，并持续每约 60 秒 heartbeat，直至"
        "提交 result 或释放 claim。progress 不续租；primary 不得代发 heartbeat。"
        "child 在预计超过 60 秒的整文件 Write、大 patch、批量编辑或"
        "命令前，必须先用 expected_command_seconds 申请覆盖租约；可拆"
        "编辑改为语义小 patch 并在分块间 heartbeat。"
        "随后持续消费 frontier，"
