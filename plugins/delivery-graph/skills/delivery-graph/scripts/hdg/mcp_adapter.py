...
SERVER_INSTRUCTIONS = (
    ...
    "Delivery turn at a time. A later-started or later-discovered Delivery waits "
    ...
    "Only then may the host create or switch to the next Delivery branch and call "
    "resume_execution_mode with the retained root ID and fingerprints. The "
    "selection is already persisted and must not be retried. Resource conflicts, "
    "dirty state, HEAD drift, or uncertain receiver release stop the switch. Never "
    ...
    "never performs Git writes. workspace_status reports worktreeProvenance with "
    ...
    "relationship must remain valid. A dirty candidate may require explicit user "
    "attribution with the exact working-tree fingerprint submitted as "
    "confirmed_dirty_state_fingerprint, but that confirmation "
    "never authorizes switching to another Delivery while dirty. workspace_status "
    ...
    "AUTOMATIC uses only CURRENT_WORKSPACE_SERIAL. If it returns current-"
    "workspace branch preparation, the later Delivery waits until the previous "
    ...
    "and returns PREPARE_CURRENT_WORKSPACE_BRANCH_THEN_RESUME_EXECUTION. The host "
    "then prepares the exact branch and calls resume_execution_mode with the "
    "retained root ID and fingerprints, without presenting the selector again. "
    "Any conflict, dirty state, HEAD drift, or uncertain release stops the "
