...
- 新 Delivery 不复用已绑定其他未结束 Delivery 的会话工作区。Codex 与并行/占用的 Claude Delivery 使用 `HOST_NATIVE_LINKED_WORKTREE`；Codex 创建 `environment=worktree` 的项目任务并消费 `worktreeSetup.hostDispatch`。Claude Code 裸 CLI 从项目目录直接启动时可使用 `EXCLUSIVE_PRIMARY_CHECKOUT`：同一 primary checkout 只绑定一个未结束 Delivery；在有效 feature 分支上可直接 prepare/freeze，在主线或 detached 状态则先执行 Controller 返回的 `CREATE_DELIVERY_FEATURE_BRANCH`，随后由同一会话用原双 fingerprint 调用 `resume_execution_mode`。Controller 不执行 Git 写操作。`worktreeProvenance` 显示实际宿主、策略、拓扑、基线选择来源与 `baseRef/baseCommit/baseHeadCommit/integrationTarget`；feature 分支只有未被其他 worktree/Delivery 使用、基线有效时才允许 adoption。已有 diff 必须让用户确认全部属于本 Delivery，再以原响应的精确 `workingTree.stateFingerprint` 作为 `confirmed_dirty_state_fingerprint` 重查；`.layered-delivery/**` 控制面产物不计入业务 dirty 状态。`projectScopes.workspaceRoot` 是 preview 时的仓库锚点；Controller 在 prepare/runtime 只读解析当前独占 primary 或同 Git common directory 下唯一匹配的 linked worktree，并返回 `verifiedProjectScopes`。receiver 的 `loop_context.projectScopes` 只包含本 Delivery 当前验证通过的实际路径，冻结锚点另存于 `projectScopeAnchors`；Loop 必须直接使用实际路径，不得自行创建、`checkout` 或 `switch` 分支。`preview_hierarchy` 仍只创建共享 `scheduler.db`、根总览和 Delivery 基线/关联文档，不绑定 workspace 或创建 Graph Run。linked worktree 复用共享控制面但获得独立 `workspaceKey`；不要复制调度数据库或启动第二套控制面。
...
8. 用户点选按钮后只调用一次 `select_execution_mode`。选择默认的 `AUTOMATIC` 时，Controller 先持久记录该次业务确认：实际开发工作区已就绪则立即 prepare/freeze。Claude Code 裸 CLI 的独占 primary 已在有效 feature 分支时直接就绪；仍在主线或 detached 状态时返回 `CREATE_DELIVERY_FEATURE_BRANCH`，取得单独 Git 授权后在当前 checkout 建立 Delivery feature 分支，再调用 `workspace_status(root_id)` 与 `resume_execution_mode`。Codex primary 或需要并行隔离的工作区返回 `worktreeSetup.hostDispatch`，宿主创建 worktree 项目任务并把 continuation prompt 与双 fingerprint 交给新会话。任何续接都不得再次展示执行方式、询问 Yes/No、改走 MANUAL 或自行拼出第三个解卡菜单。选择 `MANUAL` 时，Controller 生成 handoff、登记 `HANDOFF_READY`，宿主原样展示 `manualHandoff.receiverPrompt`；该提示词也已嵌入 handoff。接收 CLI 在实际工作区显式调用 `start_manual_handoff`，然后用 `CLAIM_MANUAL_TASK` 完成 TASK 实现，并让后续 Review 回到与自动执行相同的 `DISPATCH_LOOP`。用户直接输入修改意见时不调用选择工具，继续需求沟通；需求变化后用同一 Delivery 重新生成基线与关联文档，并使旧的待执行选择失效。
