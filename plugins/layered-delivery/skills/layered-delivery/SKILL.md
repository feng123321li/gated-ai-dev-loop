...
- 总调度上下文只消费 frontier 和路由 Loop，不在自身上下文内实现 TASK 或 Review。每个 Loop 使用独立接收上下文；自动模式下先调用 `plan_dispatch_batch`，再按 `concurrentDispatchGroups` 对同批 assignment 并发创建宿主原生 Agent。`AGENT_ANALYSIS` assignment 必须显式选择计划模型，禁止误继承总调度模型；`CURRENT_EXECUTOR_FALLBACK` assignment 则在独立子上下文中沿用宿主报告的当前 Agent/模型。无相应执行容量或无法兑现 assignment 时才生成人工交接。
...
3. 对需要展示执行建议的当前 Graph，调用 `available_agents` 和 `recommend_executors`；只转述对应节点的 Agent、当前模型、置信度、备选和原因，不把普通建议当成 claim、owner、模型切换、外部 CLI 调用授权或限额恢复机制。自动执行另行调用 `plan_dispatch_batch`，只把宿主原生 Agent 放入 `executor_inventory`，不得把 PATH 中存在的终端命令直接冒充可自动启动的宿主 Agent。对每个当前 Ready TASK/Review，优先读取 `loop_context` 并按执行说明用 Agent 分析能力自动判级，向 `node_requirements` 提交 `STANDARD`/`HIGH`、`source=PLANNING` 和原因；完成分析但不确定时使用 `HIGH`。若某些节点没有分析结果，提交与 inventory 精确匹配的宿主 `current_executor`，只让这些节点走 `UNCLASSIFIED / CURRENT_EXECUTOR_FALLBACK`，不得由 Python 补做判级；如果宿主也没有报告当前 Agent/模型，则缺失节点不可派遣。
4. 对派遣计划的每个 assignment，在该 Delivery 的 `workspaceIsolation.workspaceKey` 和 `gitBinding.branchRef` 对应宿主 worktree 内并发创建独立接收 Agent；`EXPLICIT_OVERRIDE` 创建参数必须显式使用 assignment 的 `model.id` 与适用的 `reasoningEffort`，`CURRENT_HOST_DEFAULT` 则在独立子上下文沿用 assignment 指定的当前 Agent/模型。两者都只交付 `rootId`、`nodeId` 与 `decisionFingerprint`。先创建接收 Agent、后 claim；单个创建失败不 claim 该节点，可刷新 frontier/inventory 后重算一次，仍失败则人工交接。不要跨 Delivery 复用工作区、切到其他 Delivery 分支、复制规划会话或由总调度上下文内联执行。
5. 接收方通过 MCP 调用 `loop_context`，再以自身实际 Agent ID、实际模型 ID、assignment 的 `dispatch_reasoning_class`、`dispatch_mode=AUTO` 和收到的 `dispatch_decision_fingerprint` 调用 `dispatch_loop`。控制器校验决策是否精确绑定当前 Graph、节点、Agent、模型和推理等级；回退 assignment 必须传 `UNCLASSIFIED`，实际值不得复制未采用的建议。无可用 Agent 容量或无法兑现模型选择方式时输出人工交接且不要提前 claim；未 claim 的 Loop 由接收方直接读取 frontier 后 dispatch，已暂停的 Loop 按 `RESUME_LOOP_IN_INDEPENDENT_CONTEXT` 恢复。
