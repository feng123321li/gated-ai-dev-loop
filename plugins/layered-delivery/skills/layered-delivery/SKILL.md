...
3. 对需要展示执行建议的当前 Graph，调用 `available_agents` 和 `recommend_executors`；只转述对应节点的 Agent、当前模型、置信度、备选和原因，不把建议当成 claim、owner、模型切换、外部 CLI 调用授权或限额恢复机制。
4. 对 `DISPATCH_LOOP`，优先在该 Delivery 的 `workspaceIsolation.workspaceKey` 和 `gitBinding.branchRef` 对应宿主 worktree 内用原生 Agent 创建独立接收上下文，只交付 `rootId/nodeId`；接收方通过 MCP 调用 `loop_context`，再以实际接收 Agent ID 和实际模型 ID 调用 `dispatch_loop`。这两个字段记录执行事实，不得直接复制未被宿主采用的推荐值。不要跨 Delivery 复用工作区、切到其他 Delivery 分支、复制规划会话或由总调度上下文内联执行。
5. 无可用 Agent 容量时才输出人工交接，且不要提前 claim；未 claim 的 Loop 由接收方直接读取 frontier 后 dispatch，已暂停的 Loop 按 `RESUME_LOOP_IN_INDEPENDENT_CONTEXT` 恢复。
