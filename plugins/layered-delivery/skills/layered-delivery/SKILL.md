...
- 新 Delivery 不复用已绑定其他未结束 Delivery 的会话工作区。Git 项目统一遵循 `HOST_NATIVE_LINKED_WORKTREE`：Codex 创建 `environment=worktree` 的项目任务，Claude 创建新的 worktree session，其他可信宿主提供等价任务/会话与 linked worktree。AUTOMATIC 在 primary checkout 返回机器可消费的 `worktreeSetup.hostDispatch`；宿主按 `launchPolicy=IMMEDIATE` 直接启动任务，把续接 prompt 与双 fingerprint 一并交给新会话。`manualDirectoryChangeRequired=false` 且 `coordinatorCheckoutPolicy=PRESERVE_CURRENT_CHECKOUT`：不要求用户手动 `cd`，主调度 checkout 不切分支。Controller 不执行 Git 写操作；它通过 `worktreeProvenance` 显示实际宿主、拓扑、基线选择来源、`baseRef/baseCommit/baseHeadCommit/integrationTarget`，并只在 linked worktree 的分支未被其他 worktree/Delivery 使用时允许 adoption。不能仅凭 feature 分支名复用；已有 diff 必须让用户确认全部属于本 Delivery，再以原响应的精确 `workingTree.stateFingerprint` 作为 `confirmed_dirty_state_fingerprint` 重查。`projectScopes.workspaceRoot` 是 preview 时的仓库锚点；同一 Git common directory 中存在唯一、分支与基线匹配的 linked worktree 时，Controller 在 prepare/runtime 只读解析实际路径并返回 `verifiedProjectScopes`，不因宿主物理路径变化要求重做需求 preview。receiver 的 `loop_context.projectScopes` 只包含本 Delivery 当前验证通过的实际 worktree 路径，冻结锚点另存于 `projectScopeAnchors`；Loop 必须直接使用实际路径，不得自行创建、`checkout` 或 `switch` 分支。`preview_hierarchy` 仍只创建共享 `scheduler.db`、根总览和 Delivery 基线/关联文档，不绑定 workspace 或创建 Graph Run。linked worktree 复用共享控制面，但获得独立 `workspaceKey`；不要复制调度数据库或启动第二套控制面。
...
8. 用户点选按钮后只调用一次 `select_execution_mode`。选择默认的 `AUTOMATIC` 时，Controller 先持久记录该次业务确认：实际 linked worktree 已就绪则立即 prepare/freeze；若当前是 primary checkout，则返回 `worktreeSetup.hostDispatch`、`selectionRecorded=true` 与 `selectionContinuation.tool=resume_execution_mode`。宿主必须立即消费 `hostDispatch`：Codex 创建 worktree 项目任务，Claude Desktop/agent view 创建 worktree-backed session，CLI 宿主编排器自动启动 `claude --worktree <delivery-id>`；不得把启动命令交给用户，不要求用户手动 `cd`。detached 或主线 linked worktree 只完成返回的 feature 分支动作。新会话按 continuation 调用 `workspace_status(root_id)` 与 `resume_execution_mode`，不得再次展示执行方式、询问 Yes/No、改走 MANUAL 或自行拼出第三个解卡菜单。选择 `MANUAL` 时，Controller 生成 handoff、登记 `HANDOFF_READY`，宿主原样展示 `manualHandoff.receiverPrompt`；该提示词也已嵌入 handoff。接收 CLI 在实际工作区显式调用 `start_manual_handoff`，然后用 `CLAIM_MANUAL_TASK` 完成 TASK 实现，并让后续 Review 回到与自动执行相同的 `DISPATCH_LOOP`。用户直接输入修改意见时不调用选择工具，继续需求沟通；需求变化后用同一 Delivery 重新生成基线与关联文档，并使旧的待执行选择失效。
