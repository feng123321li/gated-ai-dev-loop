...
只有本级 GROUP Review `SUCCEEDED`，该 GROUP 才成功并向父 GROUP 传播。任一级返回 `BLOCKED` 都停止向上收敛；返回 `REPLAN_REQUIRED` 时由 frontier 进入 `REPLAN_HIERARCHY`，按执行说明等待取消授权和替代图评审。TASK 节点没有外层 Review Loop。
...
Delivery Review 返回 `BLOCKED` 时外层只记录阻断，不解释 findings；返回 `REPLAN_REQUIRED` 时由 frontier 进入 `REPLAN_HIERARCHY`；只有 `SUCCEEDED` 才解锁最终用户确认。
