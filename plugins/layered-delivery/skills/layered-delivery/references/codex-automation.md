...
Codex 可以执行由任意 Agent CLI 创建并冻结的需求。冻结状态中的 `hostRuntime` 只记录方案创建宿主，不限制当前执行宿主；Plugin MCP 从当前 Codex sandbox metadata 形成实际执行宿主凭证。遇到其他宿主创建的 frozen graph 时直接从 `graph_frontier` 接续并记录统一 `HOST_NATIVE_SKILL`，不得重新 prepare/freeze、要求用户重新确认或再次输入 `$skill`。
