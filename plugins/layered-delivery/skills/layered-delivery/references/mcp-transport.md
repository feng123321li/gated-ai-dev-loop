...
投影只供人类检查和进度掌控，不反向成为调度输入。投影缺失或被冻结输入被篡改时，应保留 SQLite 权威并交给控制器重建；Agent 不要直接打开数据库推断状态，也不要自由补写控制器拥有的 Markdown。`HANDOFF_READY` 手动冻结内容包的接收 CLI 只可把实际进展与验证结果写入 progress/acceptance；控制器刷新时保留这些人工记录，handoff、overview、baseline、revisions、接口契约和双 fingerprint 均从 SQLite 冻结输入重建，不得直接修改。需求变化时回到需求会话生成新快照。
