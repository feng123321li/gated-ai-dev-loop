...
- Claude Code 把 `hostOperation=CREATE_CLAUDE_WORKTREE_SESSION` 映射为独立会话：Desktop 的新 Code session、agent view 后台 session，或由 CLI 宿主编排器自动执行 `claude --worktree <delivery-id>` 并提交 `hostDispatch.prompt`。命令是宿主实现细节，不得显示为要求用户复制执行的操作指引，也不得要求用户手动 `cd`。Plugin 的 MCP 根由启动时 `${CLAUDE_PROJECT_DIR}` 固定，因此不得只在旧会话内调用 `EnterWorktree` 后继续；新会话加载 MCP 后重新调用 `workspace_status`。宿主没有创建独立会话的 API 时必须报告 `HOST_NATIVE_WORKTREE_LAUNCH_UNAVAILABLE`，不能宣称已经自动执行。
- feature 分支名称由宿主或用户已有分支策略决定；Layered Delivery 不生成 `feature/m_lf_` 等固定前缀，只冻结实际 checkout 的本地分支名。
- `projectScopes.workspaceRoot` 在 CHOICE_READY 快照中保存 preview 时的仓库锚点，不要求宿主 worktree 与该绝对路径相同。prepare/runtime 会以 Git common directory、精确 `branchRef` 和冻结基线只读解析每个 scope 的唯一实际 worktree，并在 `verifiedProjectScopes` 中同时返回 `declaredWorkspaceRoot` 与实际 `workspaceRoot`。Loop 启动时 `loop_context.projectScopes` 使用同一批运行时验证路径，冻结锚点另以 `projectScopeAnchors` 返回；receiver 不得对锚点目录执行开发，也不得自行创建或切换分支。同仓库唯一匹配时不重新 preview；没有匹配、存在歧义或分支/基线不符时按 Controller 错误补齐 worktree，不能自行改写 frozen hierarchy。
...
- 当前工作区是 primary checkout 时，无论当前分支名是什么都返回 `DEDICATED_WORKTREE_REQUIRED / CREATE_INDEPENDENT_WORKTREE_TASK`；AUTOMATIC 响应同时给出 `hostDispatch`，宿主立即创建独立 worktree 会话，不直接绑定新 Delivery，也不切换主 checkout。linked worktree 仍位于所选主线或 detached HEAD 时返回 `FEATURE_BRANCH_REQUIRED / CREATE_DELIVERY_FEATURE_BRANCH`。
...
7. 用户点选按钮后，把选项 ID、双 fingerprint、`requiredProjectAuthorizations` 的精确项目 ID 和真实确认人一次性传给 `select_execution_mode`：
   - `AUTOMATIC`：先记录业务确认。响应返回 `automaticDispatchRequested=true` 时已完成 prepare/freeze，立刻读取 `graph_frontier`；返回 `selectionRecorded=true`、`automaticDispatchRequested=false` 和 `worktreeSetup.hostDispatch` 时，宿主按 `launchPolicy=IMMEDIATE` 创建要求的独立 worktree 任务并发送内置 prompt，不向用户输出 `cd`/启动命令。linked worktree 只缺 feature 分支时执行 `worktreeSetup.nextAction`。新会话用原双 fingerprint 调用 `resume_execution_mode`。续接工具不接受 `confirmed_by` 或项目授权，不追加第二次确认；成功后立刻读取 frontier。两条路径都不插入模型推荐或调整窗口。
