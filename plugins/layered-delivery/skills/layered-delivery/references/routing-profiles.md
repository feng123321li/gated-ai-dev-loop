...
为提升交付速度，低风险单目标需求优先使用根 Task + LIGHT，不因文件数量、用户指定开发 Skill 或同模块生成文件自动升层。LIGHT 允许说明简洁、定向测试优先和按需读取：每个字段只写满足独立执行与验收所需的最短具体事实，不复制源码、完整父计划、通用检查清单或 evidence 模板。仍不得省略独立 acceptance、精确文件授权、真实测试结果、P0/P1 检查和最终用户确认。
