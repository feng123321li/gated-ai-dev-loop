...
3. 起草层级事实卡，选择最浅合法形态：独立 Task、Capability→Task 或 Delivery→Capability→Task。为每个实际节点起草自己的 baseline 和 `developmentPlan`；`requiredSkills` 可省略或为空，也可指定任意合法 Skill catalog 标识，控制器没有硬编码 Skill，阶段只允许 `DEVELOPMENT/GATE/FINAL_REVIEW`。
