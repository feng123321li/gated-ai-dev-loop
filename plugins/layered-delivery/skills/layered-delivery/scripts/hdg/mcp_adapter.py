...
SERVER_INSTRUCTIONS = (
    ...
    "to a new host-native worktree session before preview: create a Codex "
    "project task with environment=worktree, while Claude must start a new "
    "worktree session. After AUTOMATIC is recorded in a primary checkout, "
    "worktreeSetup.hostDispatch tells the host to launch that task "
    "immediately, pass its continuation prompt and fingerprints, and do so "
    "without asking the user to change directories. The launch preserves "
    "the coordinator main or master checkout so it can dispatch more "
    "Deliveries in parallel. Because Claude fixes CLAUDE_PROJECT_DIR when "
    "its MCP "
    "starts, it must not merely EnterWorktree in the old Claude session. "
    "Codex, Claude, and future trusted host adapters implement the same "
    "HOST_NATIVE_LINKED_WORKTREE contract. workspace_status reports "
    ...
    "adoptable only in a linked worktree when no other worktree or Delivery "
    "uses it and its base relationship remains valid. A dirty candidate "
    ...
    "AUTOMATIC records that human choice before worktree validation. A ready "
    "worktree immediately prepares, freezes, and enters dispatch. A primary "
    "checkout returns host-owned worktree setup; the new worktree session "
    "calls workspace_status and resume_execution_mode, never presents the "
    "mode selector or asks for confirmation again. "
