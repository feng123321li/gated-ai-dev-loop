...
3. claim 成功后立即用精确 operation 调用 `heartbeat_loop`，早于 `loop_context` 解读及任何代码检查、文件检索、依赖分析或命令。首次返回 `leaseRenewed=false / NOT_REQUIRED` 只表示尚未进入续租阈值：保留原 `leaseExpiresAt`，不得停止本轮 heartbeat 计划。
