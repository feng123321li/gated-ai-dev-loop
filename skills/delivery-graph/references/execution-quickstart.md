...
用于自动 Graph，以及手动 handoff 接收后启动的同一冻结 Graph 的运行、恢复与阻断处理。两种模式共享 Hook 认证、claim、项目 scope、operation、heartbeat、资源锁、进度、Review、验收和恢复契约；差异只在 TASK 实现的授权来源。Codex AUTOMATIC TASK 由 `SessionStart` 或调用时 `PreToolUse` 认证的当前 Delivery 顶层会话直接领取，AUTO Review 仍绑定非空 reservation 和独立 receiver；MANUAL attestation 的 reservation 固定为 `NULL`。
...
- `CLAIM_MANUAL_TASK`：只对应 `TASK_LOOP`，来源是 `start_manual_handoff` 已启动的 manual Graph，或指定自动 TASK 的显式人工接管。总协调上下文不得实现或 claim；当前可信宿主创建独立原生 child，由 Adapter PreToolUse Hook 绑定真实 child/parent/workspace，签发 reservation 为 `NULL` 的一次性 receiver attestation，再以 `dispatch_mode=MANUAL` claim。claim 后立即独立 heartbeat，再进入完整 TASK Loop。
- `DISPATCH_LOOP`：Codex 的 READY `TASK_LOOP` 使用 Hook 上下文中的 session capability 调用 `claim_current_task`，在当前 Delivery 会话 claim 并实现；该工具拒绝 Review。所有 Review，以及其他 Adapter 仍需 child receiver 的 TASK，按当前可信宿主调用 `plan_dispatch_batch`，计划预留独立 receiver，固定 `modelPolicy=CURRENT_HOST_INHERIT`。
...
用户选择自动执行时，`select_execution_mode(AUTOMATIC)` 已记录一次业务确认。Claude Code 与 Codex 的 Git Delivery 都消费 `worktreeSetup.hostDispatch`：Claude 在同一顶层会话内创建/进入稳定 Delivery worktree，启动后台 `delivery-graph:delivery-coordinator` 后立即返回 primary；Codex 创建 `environment=worktree` 的后台项目任务。Codex 项目任务启动/恢复时，可信 `SessionStart` Hook 为精确 session/worktree 签发有时效、可轮换且数据库只存哈希的能力；该任务调用 `workspace_status(root_id)` 并以原双 fingerprint 调用 `resume_execution_mode`，随后直接执行 READY TASK。primary 不切换协调 checkout、不实现 Loop，只以 `MONITOR_ONLY` 从共享控制根读取 frontier。Review frontier 才调用 `plan_dispatch_batch` 取得 reservation 并创建独立 receiver。用户选择手动开发时仍按 handoff 协议启动 manual Run 和独立 TASK receiver。
...
receiver 完成 claim、读取 `loop_context` 并提交首次独立 heartbeat 后，可以在 Loop 内根据成本、任务和本机能力自主创建 Codex、Claude、Grok、DeepSeek 等 Worker，选择其模型、effort、并发和升级路径。内部 Worker 只向 receiver 返回工作结果，不得调用任何 Graph mutation 工具或获得 operation、attestation、reservation。新增 Worker 供应商不改变 Delivery Graph；只有要让供应商成为外层 receiver 时才需要新增可信 Adapter。
...
1. primary 总调度上下文只读取 frontier 和路由 action，不直接执行 Loop。Codex AUTOMATIC 的 Delivery worktree 当前会话在 `SessionStart` 认证后兼任 TASK receiver，但不能内联 Review。
   ...
   - `CLAIM_MANUAL_TASK` 不进入 `plan_dispatch_batch`。它来自完整 manual Graph，或来自 `handoff_ready_automatic_task` 已显式恢复的单个 READY 自动 TASK。总协调上下文创建独立宿主原生 TASK child，只传 root/node 和冻结双 fingerprint；child 的 `dispatch_loop(MANUAL)` 不带 AUTO reservation、decision、模型、reasoning class 或 HOST_NATIVE transport，但必须由当前可信 Adapter 注入 reservation 为 `NULL` 的一次性 receiver attestation。Controller 原子消费它并绑定真实 receiving context、parent、workspace 与唯一 operation；claim 后仍必须先独立 heartbeat，并遵守后续所有进度、租约、结果和恢复规则。
...
2. Codex READY TASK 以当前 Graph fingerprint 调用 `claim_current_task`；Controller 验证 session capability、Delivery worktree、TASK kind、READY attempt 和项目 scope 后，在 `AUTOMATIC` 模式内通过当前宿主会话直接 claim，完全不创建 reservation。`SessionStart` 能力存在时直接使用；Desktop 未在启动时运行它时，受信任的当前任务 `PreToolUse` 核对顶层 transcript、session 与 workspace 后按需签发等价能力。任何 Review 都拒绝该工具，改为调用一次 `plan_dispatch_batch` 并按 `concurrentDispatchGroups` 创建独立 receiver。两条入口都必须经过 Hook，能力不得复制给 Worker。
3. `claim_current_task` 成功后当前会话立即读取 `loop_context` 并 heartbeat，不再创建 child。顶层会话后续 mutation 可由 session capability 或受信任的 mutation `PreToolUse` 解析同一 live operation。对 Review assignment 仍先预留、再立即创建 receiver；Codex 由 `SubagentStart` Hook 在 child 可见前完成 host-side claim，并向该 child发放自己的 session capability。Hook 启动证明失败时沿用 reservation 立即释放和 `DELIVERY_GRAPH_STARTUP_ERROR` 规则。若当前 TASK lifecycle capability 缺失，不得无认证直领；信任 Hook 后直接重试调用时认证。
...
5. Codex `SessionStart` 和 `SubagentStart` 只接受宿主可信 sessions 根与实际 Delivery workspace。顶层 Delivery task 的 `SessionStart` capability 是 `DELIVERY_COORDINATOR`，可领取当前 TASK 并规划独立 Review；subagent 不取得该角色。AUTO Review 的 `SubagentStart` capability 是 `LOOP_RECEIVER`，只授权它已领取的 Review mutation，不能领取 TASK 或规划下一批。两者都只把 raw capability 注入对应 receiver 的 developer context，数据库只保存摘要；receiver 在允许的 claim、planning 和 mutation 中携带私有 session 字段，始终省略 `operation_id`。Controller 由 session 身份解析 live operation。Capability 不得出现在 Worker 输入、终端、进度、result 或用户消息。支持嵌套 PreToolUse 的宿主仍可叠加一次性调用证明，但不再是 code-mode 正常执行的唯一入口。Claude 和 MANUAL child 保持原 receiver attestation 流程。
6. 按 `loop.ref` 启动对应内部 TASK、TASK Review、GROUP Review 或 Delivery Review Loop，并把 `payload` 和共享 `skillHints` 原样交给该 Loop。
   - Claude Code 与 Codex 的自动 Git Delivery 都使用独立、稳定的 linked worktree。Git `workspaceKey` 由当前历史的根提交集合（repository lineage）与本地 `branchRef` 生成，不包含仓库或 worktree 绝对路径；移动仓库，或移动、重建同一分支 worktree 后身份保持不变，不同分支仍隔离。旧的路径型绑定在原路径首次可信访问时原子升级为该稳定身份。控制器继续校验冻结 `gitBinding`，禁止另一个历史 lineage 或 feature 分支冒充 Delivery。Claude Hook 以真实 cwd 的一次性证明把固定 MCP 控制根与实际执行 worktree 解耦；`loop_context` 只把本 Delivery 已验证的 linked worktree 作为有效 `projectScopes` 下发，receiver 不读取或切换另一个 Delivery 的检出分支。
...
11. 首次独立 heartbeat 之后，长任务继续按租约调用 `heartbeat_loop`。宿主未发出 child 完成通知只表示没有终态通知，既不是 heartbeat，也不能证明 receiver 仍存活；`SUSPECT_LOST` 也只证明控制面心跳和进度都静默，不能自行归因为 Maven 阻塞、会话错配或进程退出。Codex AUTOMATIC 的当前 TASK 与 Review receiver 对 heartbeat、progress、pause、result 省略 `operation_id`，由 SessionStart/SubagentStart capability 解析 live claim；Claude 与 Codex MANUAL child 仍由共享 PreToolUse 校验宿主身份并注入 operation。其他适配器在获得等价宿主授权通道前不能自动变更 Loop。检测到上下文容量压力或高轮次 Hook 摩擦且工作仍可继续时，不提交失败结果；在租约有效期内调用普通 `pause_loop`。
...
- 未 claim 且无 Agent 容量：保持 READY，不调用 `dispatch_loop` 或 `pause_loop`；仅在满足上述无领取、无 reservation、clean、用户确认条件后调用 `handoff_ready_automatic_task`，再由独立人工 receiver MANUAL claim。
