...
投影只供人类检查和进度掌控，不反向成为调度输入。投影缺失或被冻结输入被篡改时，应保留 SQLite 权威并交给控制器重建；Agent 不要直接打开数据库推断状态，也不要自由补写控制器拥有的 Markdown。`scheduler_metadata.state_contract` 固定当前 schema v3 Graph 生成契约，不兼容生成器必须拒绝共同写入同一个数据库，不能通过重算指纹或直接改库绕过。`HANDOFF_READY` 手动冻结内容包只用于接收和启动；接收宿主通过 `start_manual_handoff → graph_frontier → 独立 child 的显式 operation Loop 工具` 报告实际进展与验证结果。handoff、overview、baseline、progress、acceptance、revisions、接口契约和双 fingerprint 均由 SQLite 冻结输入与事件链重建，不得直接修改。需求变化时回到需求会话生成新快照。
