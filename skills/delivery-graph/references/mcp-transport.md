...
Codex Plugin manifest 必须显式声明 `./hooks/hooks.json`。安装或升级后在 Codex CLI 交互界面的 `/hooks` 中审查并选择始终信任当前精确 Hook 定义；Desktop 不保证弹出信任对话框。信任按定义哈希持久保存，未变更时后续任务不再提示，升级改变 Hook 定义后重新审查。`SessionStart` 在顶层 Delivery task 启动、恢复或 compact 时优先为精确 session/workspace 签发可轮换、有时效、数据库只存哈希的 coordinator capability；Desktop 未运行该生命周期 Hook 时，受信任的 `claim_current_task` `PreToolUse` 使用顶层 transcript、session 和可信 workspace 按需签发等价能力。subagent 不能取得该角色。Adapter 同时核对 capability、session role、可信请求 workspace 和 Delivery binding；缺少、伪造、跨 role/session/workspace 或过期时 fail closed。该路径不要求修改 `model_catalog_json`、模型 tool mode 或用户级编排配置。

Codex AUTOMATIC TASK 由当前顶层会话的 lifecycle 或调用时 capability 调用 `claim_current_task`，通过当前宿主会话 claim 且没有 reservation；该工具只接受 READY `TASK_LOOP`。AUTOMATIC Review 由 reservation 与 `SubagentStart` 授权，reservation 非空；MANUAL TASK 仍由 reservation 为 `NULL` 的 receiver attestation 授权。这些是两种执行模式内的接收路径，共享 operation、lease 和 mutation 校验。Codex transcript 必须位于宿主可信 `.codex/sessions` 根；session capability 不得复制给 helper、内部 Worker、自定义 `CODEX_HOME` 或其他任务。

未显式声明 `delivery.projectScopes` 的单仓 Delivery 在运行时从顶层 `delivery.gitBinding` 与经宿主证明的 Delivery workspace 合成唯一 `primary` scope；多仓仍逐项验证显式 scope。AUTO Review 的 `SubagentStart` 在真实 transcript 中已定位 reservation、但 receiver/workspace/scope 验证失败时，只有节点仍 READY 且尚未签发 receiver 身份才立即把 reservation 标记为过期并记录启动失败；child 收到 `DELIVERY_GRAPH_STARTUP_ERROR` 后不得调用 Loop 工具或访问仓库，只能报告协调器。Session capability 只进入对应 receiver 的 developer context，严禁出现在用户输出、日志、进度、result 或 Worker 输入。
