...
协调工作区不是默认的唯一写入范围。每个参与工作区都必须在当前轮次的授权清单中单独列出。所有路径都使用规范化绝对根路径；业务允许路径使用相对于对应根目录的路径或安全 glob。不要依赖当前工作目录、盘符假设、符号链接别名或“相邻仓库”等隐含约定。
...
`preExistingChanges` 的条目沿用单工作区 schema v1：保存路径、状态码和可用的工作树哈希，不读取敏感文件内容。当前 CLI 的 `gated-loop self-check` 只处理单工作区 schema v1；在 CLI 尚未支持 schema v2 时，宿主必须按本文件逐工作区执行等价检查并聚合证据，不得用一次单仓库命令声称多工作区门禁通过。
