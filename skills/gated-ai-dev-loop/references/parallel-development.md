...
任一条件不满足时使用 `single`。不要为了并行而拆分强耦合任务；公共契约、共享文件和跨组集成改动必须由一个任务组独占，依赖它的任务进入后续波次。
...
`maxConcurrency` 不得超过独立任务组数量或宿主可证明的运行容量。只有同一波次且 `dependsOn` 为空的分组可以并发。宿主验证路径集合不重叠后再启动。
...
每个 Agent 可以读取完成任务所需的仓库内容，但只能写自己的精确路径集合。采用以下一种可证明策略：
...
2. 分别检查真实改动是否只落在该 Agent 的允许路径。
3. 拒绝重叠路径、未归属改动和被保护文件。
4. 机械应用无冲突结果；记录 `integration-result.json`。
5. 对集成后的完整 diff 重新分类、执行全部冻结测试并生成 `self-check-report.md`。
6. 只有聚合机械门禁通过后，才把完整 diff、并行计划、归属证据和集成结果交给与开发者分离的全新只读其他 Agent；没有其他 Agent 时使用全新验收子 Agent。两者都不得继承任何并行开发上下文，并生成统一的 P0/P1/P2 验收报告。
