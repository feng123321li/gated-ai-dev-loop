...
任务恢复后的 `nextAction` 可以直接进入任一对应阶段，不要求从图 1 重新开始。
...
flowchart TD
    ...
    TASK_SELECT --> RESUME
    RESUME --> RECOVERED_ACTION["读取磁盘 nextAction<br/>不得重新 start 或替换 baseline"]
    ...
    CONFIRM -->|否| HOST
    CONFIRM -->|是| FREEZE["冻结唯一开发授权与指纹<br/>进入图 2"]

    classDef host fill:#dbeafe,stroke:#2563eb,color:#172554;
    classDef gate fill:#ffedd5,stroke:#ea580c,color:#431407;
    classDef human fill:#f3f4f6,stroke:#4b5563,color:#111827;

    class INPUT,HOST,RECOVER,RESUME,TASK_SELECT,RECOVERED_ACTION,NEXT_ACTION,STAGE1,STAGE2,STAGE3,STAGE4,ROUTE,SCALE,MICRO_LIGHT,MICRO_FULL,TASK,CAPABILITY,PROJECT,KIND,RUNTIME,TRACK host;
    class FREEZE gate;
...
flowchart TD
    ...
    DEV_RESULT -->|COMPLETED| WRITEBACK["确认最终状态与证据<br/>已回写 progress.md"]
    DEV_RESULT -->|BLOCKED| BLOCKED["回写阻断事实与解除条件<br/>保持 BLOCKED"]
    BLOCKED --> RESUME_BLOCKED["解除后回图 1 恢复任务<br/>按磁盘 nextAction 续接"]
    SINGLE_DEV -.-> PROGRESS_NOTE["过程中的每次 T / SOP 状态转换<br/>都立即回写 progress.md 与证据"]
...
```mermaid
flowchart TD
    START["来自图 2<br/>开发结果与最新进度证据"] --> GATES["机械门禁并生成 self-check-report.md<br/>按依赖波次逐工作区检查并聚合"]
    ...
    OTHER_REVIEW --> REVIEW_RESULT{"生成轮次 P0 / P1 / P2 报告<br/>刷新根级最终验收报告"}
    SUBAGENT_REVIEW --> REVIEW_RESULT
    HUMAN_REVIEW --> HUMAN
    REVIEW_RESULT -->|FAIL，未超过三轮| REPAIR
    REVIEW_RESULT -->|FAIL 已达上限<br/>或证据不足| HUMAN
    REVIEW_RESULT -->|PASS| FINAL["进入图 4<br/>用户最终确认"]
    ...
    class REVIEW_PLAN,REVIEWER,OTHER_REVIEW,SUBAGENT,SUBAGENT_REVIEW,REVIEW_RESULT,FINAL review;
    class HUMAN_REVIEW,HUMAN human;
```
...
```mermaid
flowchart TD
    START["来自图 3<br/>语义验收 PASS"] --> FINAL_CONFIRM{"用户最终确认？"}
    FINAL_CONFIRM -->|确认完成| COMPLETE["完成"]
    FINAL_CONFIRM -->|等待反馈| WAIT["保持待确认<br/>不自动发布"]
    FINAL_CONFIRM -->|修改、建议或新目标| FEEDBACK{"先恢复原任务并分类<br/>WAITING_FOR_FEEDBACK_CONFIRMATION"}

    FEEDBACK -->|冻结 R / A / T 已要求| REPAIR_CURRENT["REPAIR_CURRENT<br/>同任务最小修复，回图 2"]
    FEEDBACK -->|改变目标、范围或验收| REVISION_CONFIRM["确认 REVISION_OF 与新基线<br/>保留原任务包"]
    REVISION_CONFIRM -->|确认| HOST["回图 1<br/>建立关联修订包并重新冻结"]
...
- 恢复任务后按磁盘 `nextAction` 进入对应阶段，不得重新 `start` 或替换冻结 baseline。
...
- `development-overview.md` 提供稳定任务地图，`progress.md` 在每次状态转换后更新；人工优先查看根级 `final-acceptance-report.md`，再按需追溯总览、进度和轮次证据。
