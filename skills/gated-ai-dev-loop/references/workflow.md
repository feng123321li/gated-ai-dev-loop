...
flowchart TD
    ...
    ACTIVE_HOST -->|Codex| CODEX_DEV["全新 Codex 开发上下文"]
    ACTIVE_HOST -->|Claude| CLAUDE_DEV["全新 Claude 开发上下文"]
    DEV_MODE -->|manual| HANDOFF["输出项目路径、冻结包、提示词和返回方式"]
    HANDOFF --> USER_DEV["用户在全新 Codex 或 Claude 中开发"]

    CODEX_DEV --> ACTIVE_RESULT{"主动调用结果"}
    CLAUDE_DEV --> ACTIVE_RESULT
    ACTIVE_RESULT -->|成功| DEV_RESULT["COMPLETED / BLOCKED 事实"]
    ACTIVE_RESULT -->|失败且确认零写入| RESELECT["展示失败事实，推荐 manual"]
    RESELECT --> MODE_WAIT
