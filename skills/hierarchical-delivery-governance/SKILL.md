...
10. 需求根 `progress.md` 使用 Markdown 表格展示整树明细：第一列保留与 `development-plan.md` 相同的工作项 ID、父子顺序和层级，其余列分别展示阶段、状态、门禁、认领、节点文件和阶段性产物。每次控制器写回都会从 registry 自动重建该表格，不依赖 Agent 手工改表。
