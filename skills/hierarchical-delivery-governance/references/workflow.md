...
2. 只读恢复 registry；不存在时表示尚未持久化工作项。
...
7. 内置控制器持久化并校验 `development-mode.json` 后，才计算 READY、生成独立上下文并原子认领；manual 模式必须把 `task-context --json` 返回的 `handoffPrompt` 原样展示，不能只给文件链接。
