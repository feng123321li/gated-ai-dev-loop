...
4. 对 `DISPATCH_LOOP`，优先在该 Delivery 的 `workspaceIsolation.workspaceKey` 和 `gitBinding.branchRef` 对应宿主 worktree 内用原生 Agent 创建独立接收上下文，只交付 `rootId/nodeId`；接收方通过 MCP 调用 `loop_context` 和 `dispatch_loop`，不要跨 Delivery 复用工作区、切到其他 Delivery 分支、复制规划会话或由总调度上下文内联执行。
