...
- 总调度上下文只消费 frontier 和路由 Loop，不在自身上下文内实现 TASK 或 Review。每个 Loop 使用独立接收上下文；自动模式下先调用 `plan_dispatch_batch` 原子取得短租约 `dispatchReservationId`，再按 `concurrentDispatchGroups` 对同批 assignment 并发创建宿主原生 Agent。第二个调度器只能看到 `WAIT_FOR_DISPATCH_RECEIVER`，不得重复创建。`AGENT_ANALYSIS` assignment 必须显式选择计划模型，禁止误继承总调度模型；`CURRENT_EXECUTOR_FALLBACK` assignment 则在独立子上下文中沿用宿主报告的当前 Agent/模型。创建动作必须走当前宿主正式 Agent API；跨宿主 Agent 只有在宿主提供同样受控的原生 Broker 时才可标为 `HOST_NATIVE`，否则保持节点未 claim 并生成人工交接。不得为了异构 Review 从 Claude Code 启动 Codex CLI，或从 Codex 启动其他外部自治 CLI。安全回退到同 Agent/模型时只能称为独立上下文审查，并如实保留 `diversityLevel=CONTEXT_ONLY`，不能声称跨 Agent/跨模型独立。
...
5. 接收方通过 MCP 调用 `loop_context`，再以自身实际 Agent ID、实际模型 ID、assignment 的 `dispatch_reasoning_class`、`dispatch_mode=AUTO`、`dispatch_transport=HOST_NATIVE`、`dispatch_reservation_id` 和收到的 `dispatch_decision_fingerprint` 调用 `dispatch_loop`。控制器校验预留和决策是否精确绑定当前 Graph、节点、attempt、Agent、模型、推理等级和派遣通道；外部进程或没有有效预留的接收方不能提交 AUTO claim。回退 assignment 必须传 `UNCLASSIFIED`，实际值不得复制未采用的建议。无可用 Agent 容量或无法兑现模型选择方式时输出人工交接且不要提前 claim；未 claim 的 Loop 由接收方直接读取 frontier 后 dispatch，已暂停的 Loop 按 `RESUME_LOOP_IN_INDEPENDENT_CONTEXT` 恢复。
