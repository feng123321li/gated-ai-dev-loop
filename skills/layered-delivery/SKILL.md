...
- `available_agents` 与 `recommend_executors` 只提供当前主机的动态发现和建议。`recommend_executors.temporarily_unavailable_agent_ids` 可在一次刷新中临时排除已知限额执行者，但不持久化。建议不进入 schema v3、Frozen Graph、SQLite、claim 或 owner，工具自身不启动 CLI、切换模型或派遣 Loop；自动执行模式的总调度器可以消费建议并通过宿主原生 Agent 创建接收上下文。
...
- 严格区分执行容量状态：未 claim 且无 Agent 容量时只生成人工交接；已 claim、租约有效且出现上下文或 Hook 压力时调用普通 `pause_loop`；单个执行 Agent 命中提供方 Token 限额时，使用真实 `resetAt` 调用 `pause_loop(resume_at=..., capacity_scope=EXECUTOR)`，先重新推荐独立候选、无候选再定时等待；调度宿主自身受限时，由宿主适配器调用 `pause_loop(resume_at=..., capacity_scope=HOST)` 并注册不依赖模型的定时唤醒，不在限额窗口内调用推荐器；租约过期时调用 `advance_graph`，禁止 pause。由宿主派出的某个原生 Agent 单独受限仍属于 `EXECUTOR`。容量交接和限额等待都不提交 Loop outcome。
...
2. 对需要展示执行建议的当前 Graph，调用 `available_agents` 和 `recommend_executors`；只转述对应节点的 Agent、当前模型、置信度、备选和原因，不把建议当成 claim、owner、模型切换或外部 CLI 调用授权。处理 `RECOMMEND_ALTERNATE_OR_WAIT` 时，把 action 中的 `excludedOwner` 作为 `temporarily_unavailable_agent_ids` 刷新该节点建议。
...
8. 长运行在租约到期前调用 `heartbeat_loop`；只有租约仍有效时，上下文容量压力或高轮次 Hook 摩擦才使用普通 pause/handoff。开发中命中提供方 Token 限额时，从真实错误读取 `resetAt`；执行 Agent 受限用 `capacity_scope=EXECUTOR`，调度宿主自身受限用 `capacity_scope=HOST`。不得猜测恢复时间。
9. 对 `RECOMMEND_ALTERNATE_OR_WAIT`，立即临时排除原执行者并刷新推荐。存在不同且真实可用的宿主 Agent 时，立即 `resume_loop` 并自动派遣同一 Loop attempt；没有候选时按 frontier 的 `nextWakeAt` 使用宿主原生定时唤醒，届时重新调用 frontier 和推荐，再自动派遣。对 `WAIT_FOR_HOST_CAPACITY` 不调用推荐器；模型外宿主定时器必须在 `nextWakeAt` 唤醒并重新消费 frontier。推荐工具和控制器都不会自行启动进程。租约已过期时停止旧 operation，由 frontier/`advance_graph` 回收。
...
- `RECOMMEND_ALTERNATE_OR_WAIT`：先刷新排除限额执行者后的推荐；有独立候选则提前恢复并派遣，无候选则等待 `nextWakeAt`。到时 frontier 自动恢复同一 attempt；宿主必须使用原生定时唤醒机制再次消费 frontier。
- `WAIT_FOR_HOST_CAPACITY`：调度宿主自身不可调用模型。不要刷新推荐；由模型外宿主定时器在 `nextWakeAt` 唤醒并重新消费 frontier。若宿主没有这种定时能力，只能等待下一次外部或用户唤醒，不能宣称全自动恢复。
