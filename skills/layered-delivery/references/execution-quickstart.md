...
3. 按 `loop.ref` 启动对应内部 Task Loop，并把 `payload` 和共享 `skillHints` 原样传入。
4. 内部 Loop 先识别当前任务与宿主可用 Skill，再优先原生触发适用的 Skill Hint；不要因为 hierarchy 提供了提示，就假定每条提示都适用于当前 Loop。
5. 让内部 Loop 自己选择其他必要 Skill、创建文件、运行测试、执行 Gate 和处理 Gate→development 修正。
6. 长任务持续调用 `heartbeat_loop`。需要移交时先 `pause_loop`，接收方再 `resume_loop` 和重新 dispatch。
7. 用 `record_loop_result` 提交标准结果。

`predecessors` 表示 Graph 直接前驱；`upstreamLoopResults` 提供所有传递上游 Task Loop 的不透明结果，供依赖 Loop 和 Review Loop 消费。Join 自身没有业务 result，不能用 Join 的空 outcome 替代其上游 Loop 结果。
...
已 claim Loop 占用其全部 `resourceClaims`。另一个 Ready Loop 只要存在相同键就不能 dispatch；无交集则可并行。不要从路径、仓库层级或模块前缀推导额外冲突。
