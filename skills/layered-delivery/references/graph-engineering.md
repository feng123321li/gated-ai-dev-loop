...
用户不直接定义任意图节点或边。`prepare_hierarchy` 将通过校验的层级结构编译成一张图，并提供两种有类型的视图：
...
准备阶段同时返回 `hierarchyFingerprint` 和 `graphFingerprint`。一次 `freeze_hierarchy` 确认会冻结完整层级、开发方式和编译后的图。
...
使用 MCP `graph_frontier`，传入根或子树工作项 ID。
...
`ready_tasks` 只返回当前 `DISPATCH_TASK` 动作中的 `workItemId`，用于兼容性只读查看。执行适配器不得另行实现第二套就绪判断或从中人工挑选；实际执行以完整 `dispatchPlan` 为准。
...
每个 claim 都包含 `claimedAt`、`lastHeartbeatAt` 和 `leaseExpiresAt`。默认软租约为 30 分钟、心跳间隔为 5 分钟、竞争宽限为 2 分钟。MCP `dispatch_task` 与 `heartbeat_task` 返回 `leasePolicy`，其中含 `heartbeatDueAt`、`leaseExpiresAt`、`hardExpiresAt` 和精确动作提示；租约迁移使用 `heartbeat_task` 与 `advance_graph`。
...
`heartbeat_task` 可延长匹配且尚未硬过期的 operation。软租约到期后的 2 分钟内，如果 `advance_graph` 尚未完成失联迁移，同一 operation 仍可补心跳或提交结果；事务先到者生效。硬到期后 frontier 返回正式 `ADVANCE_GRAPH` 动作，执行循环必须调用 `advance_graph` 写入 `CLAIM_LEASE_EXPIRED`、归类为 `WORKER_LOST`，然后重新查询 frontier，按预算用全新 operationId 重新认领并提交已经完成的工作，或在 `RETRY_EXHAUSTED` 后请求干预；这不是人工重置。旧 operationId 在同一 graph run 中禁止复用，因此失联 worker 的迟到结果不能污染新 attempt。控制器不会用后台 daemon 假装 worker 存活，也不会猜测任意业务失败是否可以重试。

显式运行控制使用 MCP `pause_task`、`resume_task` 和需要用户交互授权的 `cancel_graph_run`。

暂停和恢复沿用同一个 attempt。取消是经过明确确认的 graph run 终止迁移，不属于失败重试。

## 状态与事件

状态与事件只使用 MCP `graph_status`、`graph_events` 和 `graph_replay`。

`graph_status` 返回图指纹、冻结的运行时策略、graph run、有类型的节点和边，以及节点当前状态、attempt、owner、operationId、claim 租约、最近迁移、失败分类、尝试耗尽状态和阻断原因。

MCP `graph_events` 使用 `after_event_id` 与 1–200 的 `limit` 分页，`hasMore=true` 时继续提交 `nextCursor`。Server 验证从链首到当前页的哈希连续性，但内存只保留当前页。正常生命周期事件包括图启动、Task 认领/心跳/结果、租约过期、暂停/恢复、门禁结果、重试/耗尽、审查、最终确认、同合同修正失效传播和取消。
...
`graph_replay` 从 `GRAPH_RUN_STARTED` 开始应用完整事件流，重建每个 node attempt 和 graph 状态，计算 replay fingerprint，并报告事件回放结果与 graph/node run 快照之间的差异。出现差异时，正常状态和 frontier 查询必须阻断。

如果事件链和证据链校验通过，只有查询快照损坏，可以在明确用户确认后调用 MCP `rebuild_graph_run`。该工具从事件重建 graph/node run 快照，并记录恢复交互；它不会修改冻结图、事件或 evidence。
...
当自动恢复类失败在第 3 次 attempt 仍未成功时，控制器写入 `RETRY_EXHAUSTED`，将节点标记为尝试耗尽，并阻断 graph run。`retry_item` 仍用于符合条件的门禁或人工恢复场景，但同样受当前节点的 3 次 attempt 预算约束。Task gate 重试会同时创建新的 execution 与 gate attempt，让修复重新经过认领、结果写回和门禁；协调节点 gate 只重试自身。普通的可重试 Task 失败由 Graph 自动路由，不再要求执行平台手动调用它。

新的 Task 派发不得复用当前 graph run 中任何历史 attempt 的 operationId，控制器机械拒绝复用。

## 同合同修正与失效传播

只有目标、需求、验收、接口、数据合同、测试命令、拓扑和外部权限均保持不变时，才允许执行 `remediate_task`。
