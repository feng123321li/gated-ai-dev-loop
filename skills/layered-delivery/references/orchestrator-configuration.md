...
| 跨 Adapter 调度 | 关闭 | 默认只在能够确定的当前宿主 Adapter 内派遣 |
...
`allowCrossAdapterDispatch=true` 只是用户授权。只有中央编排器能够通过正式宿主
Adapter API 创建目标上下文、验证可用容量并签发同一编排根下的接收证明时，
目标才可进入自动 assignment。PATH 中发现的 CLI、外部进程和仅安装但未认证的
Adapter 不会因此升级为可自动派遣能力。
...
- `false`：只选择能够确定的当前 Adapter；多候选但无法确定当前 Adapter 时
  fail closed。
- `true`：可以在 `allowedAdapters` 范围内选择多个可信宿主 Adapter。
...
| `SWITCH_ADAPTER` | 允许中央编排器在跨 Adapter 已开启时尝试其他可信 Adapter |
