        ...
        "projectionGuidance": {
            "executionInteraction": {
                ...
                "description": (
                    ...
                    "reconstruct, rewrite, or add options. Selecting either "
                    "AUTOMATIC or MANUAL is recorded once and fixes execution to "
                    "CURRENT_WORKSPACE_SERIAL. One actual workspace runs one "
                    "Delivery turn at a time. A later Delivery with either "
                    "recorded selection is marked QUEUED with a mode-specific "
                    "continuation and waits until the prior run is "
                    "paused, terminal, or ready for final user confirmation; "
                    "every frozen writable branch must then have a verifiable "
                    "business commit, a clean work tree and index, unchanged "
                    "frozen HEAD binding, and safe quiescence of every receiver "
                    "and reservation. The Controller must persist "
                    "WORKSPACE_TURN_RELEASED; status alone is not release. "
                    "Releasing at the confirmation boundary does not complete "
                    "that Delivery. "
                    "Only then does the host mechanically stash pre-existing "
                    "business changes when needed, create or switch the exact "
                    "Delivery branch, and call resume_execution_mode for AUTOMATIC "
                    "or start_manual_handoff for MANUAL with the retained root ID, without "
