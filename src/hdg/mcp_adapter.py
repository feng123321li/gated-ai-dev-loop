...
SERVER_INSTRUCTIONS = (
    ...
    "Revision. An occupied workspace rejects another initial prepare before "
    "writing and requires a host-native independent worktree task. "
    ...
    "native model tiers; after manual handoff is chosen, it may accept an "
    "explicit different development Agent, and the receiver regenerates "
    "its local native-model list. Both modes are read-only, never start a "
    "CLI, switch a model, claim a Loop, dispatch work, or persist an "
    "assignment, and require no second user confirmation. Automatic "
