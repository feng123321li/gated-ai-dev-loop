...
SERVER_INSTRUCTIONS = (
    ...
    "CHOICE_READY or legacy HANDOFF_READY draft. A new user requirement defaults to a new "
    ...
    "order is quiesce, reach an eligible state, commit and clean the frozen "
    "branches, protocol recheck and persisted release, then branch switch. A "
    "released PAUSED run requeues at the tail and captures a fresh clean turn "
    "start before resume_loop makes its node READY. Only then may the host "
    "prepare the next Delivery. The recorded AUTOMATIC "
    ...
    "A status mutation never authorizes branch preparation; wait for persisted "
    "WORKSPACE_TURN_RELEASED and workspaceRelease=RELEASED. Any unmerged "
    "conflict, owner dirty state, HEAD drift, or uncertain release "
    "stops the "
    ...
    "also embedded in that file. It atomically registers HANDOFF_READY, the MANUAL "
    "selection, and current workspace queue binding without starting a Graph run. "
    "The receiving CLI must call "
    ...
    "refreshes an unstarted legacy Graph runtime policy when its hierarchy is "
    ...
    "graph_frontier or graph_status back-to-back, and the "
    ...
    "current language. Estimate the duration of every potentially blocking "
    "foreground operation before starting it, including whole-file writes, "
    "large patches, bulk edits, tests, and builds. Prefer semantic edit "
    "chunks with a heartbeat between chunks over rewriting an existing large "
    "file in one host tool call. Any atomic host tool call expected to exceed "
    "60 seconds must first request a bounded covering lease with "
    "heartbeat_loop(expected_command_seconds=...). Estimate a test or build "
    "command "
    "before starting it and prefer narrowing its scope (single module, "
    "selected tests, offline dependency resolution) over accepting a long "
    "block; any command expected to exceed 60 seconds must instead "
    "run non-blocking. A long-running test or build must use a non-blocking "
    ...
    "completion notification is not a heartbeat. Every claimed Loop must "
    "heartbeat immediately after claim, before interpreting loop_context or "
    "performing repository work, and then about every 60 seconds throughout "
    "code inspection, file search, dependency analysis, builds, tests, and "
    "Review until its result is recorded or the claim is released. "
    "NOT_REQUIRED preserves the current lease expiry and never cancels the "
    "next scheduled heartbeat. Progress is separate, never renews a lease, "
    "and never changes the heartbeat schedule. The primary dispatcher must "
    "not send heartbeats for a receiver. "
    "Heartbeat remains liveness-only: it refreshes the live Agent progress "
    "panel, renews the short lease only inside its renewal threshold, and "
    "does not rewrite Markdown projections. Raw graph events remain "
    "diagnostic-only. "
    ...
    "start_manual_handoff in the bound workspace. Its outer context only "
    ...
    "Delivery-frozen. record_loop_result captures a verified, diff-free "
...
CODEX_SERVER_INSTRUCTIONS = (
    ...
    "primary plans, routes, and monitors only. AUTOMATIC and MANUAL use "
...
_PROFILE_INSTRUCTIONS: Final = {
    ...
    RECEIVER_TOOL_PROFILE: (
        ...
        "Review Loop. Claim only the assigned node, heartbeat immediately, then "
